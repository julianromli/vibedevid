...
class PluginManager {
  ...
  async loadPlugin(pluginId: string): Promise<Plugin> {
    ...
    console.log(
      `Loaded plugin: ${plugin.metadata.name} (${plugin.metadata.version})`,
    )
...
class PluginLoader {
  async loadPlugin(pluginInfo: PluginInfo): Promise<Plugin> {
    switch (plugin.installation.type) {
      ...
      default:
        throw new Error(
          `Unsupported installation type: ${plugin.installation.type}`,
        )
    }
  }
  ...
  private async loadNpmPlugin(pluginInfo: PluginInfo): Promise<Plugin> {
    try {
      ...
      await this.executeCommand(
        `npm list ${pluginInfo.registry_name || pluginInfo.name}`,
      )
      ...
      const plugin: Plugin =
        (await pluginModule.default) || pluginModule.default.default
  ...
  private async loadGitPlugin(pluginInfo: PluginInfo): Promise<Plugin> {
    ...
    await this.executeCommand(`git clone ${repo} ${tempDir}`)
...
const SlackIntegrationTool: ToolDefinition = {
  ...
  execution: {
    requires_droid: true,
    command_template:
      'curl -X POST -H \'Content-Type: application/json\' -d \'{"message": "{message}"}\' {webhook_url} {channel_id}',
    timeout_seconds: 30,
    retry_policy: {
      max_attempts: 3,
      delay_seconds: 5,
...
const DatabaseMigrationTool: ToolDefinition = {
  ...
  execution: {
    requires_droid: true,
    command_template:
      "cd {project} && {migration_file} {dry_run ? '--dry-run' : ''} && npm test && npm run migration-test && npm test && {backup ? 'backup-db.sh' : ''}",
    timeout_seconds: 600,
    retry_policy: {
      max_attempts: 1,
    },
...
class PermissionManager {
  ...
  async validatePermissions(
    ...
    const risk_score = this.calculatePermissionRiskScore(missing, excess)
...
class PluginRegistryService {
  ...
  async getTrendingPlugins(): Promise<Plugin[]> {
    ...
    return plugins.sort((a, b) => {
      b.downloads - a.downloads || b.rating - a.rating
    })
  }
}
```
