...
class SecurityComplianceGate implements QualityGate {
  ...
  private async checkSensitiveDataExposure(
    output: TaskOutput,
  ): Promise<CheckResult> {
  ...
  private async checkAuthenticationSecurity(
    ...
    const authFiles = output.files_created.filter(
    ...
    if (issues.length > 0) {
      return {
        ...
        recommendation:
          'Use bcrypt for password hashing, implement rate limiting, and secure cookie settings',
      }
    }
  ...
  private async checkDependencySecurity(
    ...
    const packageJsonFile = output.files_created.find((f) =>
    ...
    for (const [pkg, vulnerableVersions] of Object.entries(
      ...
      if (packageJson.dependencies?.[pkg]) {
        ...
        if (
          vulnerableVersions.some((v) => version.startsWith(v.replace('<', '')))
        ) {
...
class CodeQualityGate implements QualityGate {
  ...
  private async checkCodeDuplication(
    ...
    const duplicationPercentage =
      codeBlocks.length > 0 ? (duplicates.length / codeBlocks.length) * 100 : 0
  ...
  private async checkCodeStyle(output: TaskOutput): Promise<StyleResult> {
    ...
    for (const file of output.files_created) {
      ...
      const hasInconsistentIndent = indentations.some(
        (indent, i) => i > 0 && indent > 0 && indent % 2 !== 0,
      )
...
class TestCoverageGate implements QualityGate {
  ...
  private async analyzeCoverage(output: TaskOutput): Promise<CoverageMetrics> {
    const testFiles = output.files_created.filter(
      (f) => f.includes('.test.') || f.includes('.spec.'),
    )

    const sourceFiles = output.files_created.filter(
      (f) =>
        !f.includes('.test.') &&
        !f.includes('.spec.') &&
...
class PerformanceGate implements QualityGate {
  ...
  private async checkDatabaseQueries(
    ...
    const dbFiles = output.files_created.filter(
      (f) =>
  ...
  private async checkNPlusOneQueries(
    output: TaskOutput,
...
class IntegrationValidationGate implements QualityGate {
  ...
  private async checkAPIContracts(output: TaskOutput): Promise<APICheckResult> {
    const frontendFiles = output.files_created.filter(
      (f) => f.includes('components') || f.includes('pages'),
    )

    const backendFiles = output.files_created.filter(
      (f) =>
  ...
  private async checkTypeConsistency(
    output: TaskOutput,
  ): Promise<TypeCheckResult> {
    const typeFiles = output.files_created.filter(
      (f) => f.includes('types') || f.includes('interfaces'),
    )
  ...
  private async checkSchemaAlignment(
    output: TaskOutput,
  ): Promise<SchemaCheckResult> {
    const migrationFiles = output.files_created.filter(
      (f) => f.includes('migration') || f.includes('schema'),
    )

    const modelFiles = output.files_created.filter(
      (f) => f.includes('model') || f.includes('entity'),
    )
  ...
  async runAllGates(
    ...
    for (const [gateId, gate] of this.gates) {
      ...
      try {
        ...
        console.log(
          `${result.passed ? '✅' : '❌'} ${gate.name}: ${result.score}/10`,
        )
      } catch (error) {
