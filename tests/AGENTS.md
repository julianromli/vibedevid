...
End-to-end (E2E) tests and unit tests for VibeDev ID using Playwright. Validates critical user flows, authentication, analytics, and blog functionality.
...
bunx playwright test views-tracking.spec.ts

# Run unit tests
bunx playwright test unit/blog-actions.spec.ts
...
```
tests/
├── views-tracking.spec.ts         # Analytics tracking E2E tests
├── unit/
│   └── blog-actions.spec.ts       # Blog actions unit tests
└── ...other test files
```

### Naming Rules

- **E2E test files**: `feature-name.spec.ts` (e.g., `views-tracking.spec.ts`)
- **Unit test files**: `feature-name.spec.ts` in `unit/` subdirectory
...
test.describe('Project Views Tracking', () => {
  ...
  test('should not track duplicate views from same session', async ({
    page,
  }) => {
...
describe('Blog Actions', () => {
  test('should validate blog action input', async ({ page }) => {
    // Test blog action validation through UI
...
- Blog actions: [`unit/blog-actions.spec.ts`](unit/blog-actions.spec.ts)
...
```bash
# Find all test files
bunx find tests -name "*.spec.ts"

# Run specific test suite
bunx playwright test --grep "Views Tracking"

# Run tests matching pattern
bunx playwright test --grep "authentication"

# Run unit tests only
bunx playwright test unit/
...
- **Dev server must be running**: Tests expect dev server to be active (start with `bun dev`)
- **Browser installation**: Run `bunx playwright install` after cloning repo
...
- [ ] Add blog-specific E2E tests
- [ ] Add comments section tests
