...
describe('HeroSection', () => {
  ...
  it('links the announcement banner to the WhatsApp community', () => {
    ...
    expect(screen.getByText('Announcement').closest('a')).toHaveAttribute('href', 'https://wa.vibedeveloper.id')
  })

  it('links Join Community to the given join href', () => {
    render(
      React.createElement(HeroSection, {
        joinHref: 'https://wa.vibedeveloper.id',
        handleViewShowcase: vi.fn(),
      }),
    )

    expect(screen.getByRole('link', { name: /join community/i })).toHaveAttribute('href', 'https://wa.vibedeveloper.id')
  })
})
