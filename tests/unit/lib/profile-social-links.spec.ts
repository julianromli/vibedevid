...
describe("profile social link normalization", () => {
  it("normalizes Threads URLs to the canonical @handle path", () => {
    expect(normalizeProfileSocialUrl("threads", "threads.net/jane")).toBe(
      "https://www.threads.net/@jane",
    );
    expect(normalizeProfileSocialUrl("threads", "https://www.threads.net/@jane/")).toBe(
      "https://www.threads.net/@jane",
    );
  });

  it("treats social handles containing periods as handles", () => {
    expect(normalizeProfileSocialUrl("instagram", "jane.doe")).toBe(
