...
const SKIP_PARTS = [
  `${path.sep}admin-kit${path.sep}`,
  `${path.sep}app${path.sep}routes${path.sep}`,
];
...
function migrate(content, filePath) {
  ...
  mark(
    content.replace(/import\s*\{([^}]*)\}\s*from\s*['"]next\/cache['"]/g, (m, imports) => {
      const names = imports
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
  ...
  if (content.includes("from 'next/navigation'") || content.includes('from "next/navigation"')) {
    ...
    if (m) {
      ...
      const lines = [];
      if (tanstack.length)
        lines.push(`import { ${tanstack.join(", ")} } from '@tanstack/react-router'`);
      if (nav.length) lines.push(`import { ${nav.join(", ")} } from '@/lib/navigation'`);
      mark(
        content.replace(/import\s*\{[^}]+\}\s*from\s*['"]next\/navigation['"]/g, lines.join("\n")),
      );
    }
  }
  ...
  mark(
    content.replace(
      /const\s+tCommon\s*=\s*useTranslations\(([^)]*)\)/g,
      "const { t: tCommon } = useTranslation($1)",
    ),
  );
  mark(
    content.replace(
      /const\s+t(\w+)\s*=\s*useTranslations\(([^)]*)\)/g,
      "const { t: t$1 } = useTranslation($2)",
    ),
  );
  ...
  mark(
    content.replace(
      /import\s*\{\s*NextIntlClientProvider\s*\}\s*from\s*['"]next-intl['"]\s*\n/g,
      "",
    ),
  );
  ...
  mark(
    content.replace(
      /router\.replace\(`([^`]+)`\)/g,
      "router.navigate({ to: `$1`, replace: true })",
    ),
  );
  mark(
    content.replace(/router\.replace\(([^)]+)\)/g, "router.navigate({ to: $1, replace: true })"),
  );
  ...
  mark(
    content.replace(/(<Image[^>]*?)\s+fill(\s*[^>]*?>)/g, (m, before, after) => {
