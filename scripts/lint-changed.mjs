...
function getChangedFiles() {
  const unstaged = parseLines(
    run("git -c core.safecrlf=false diff --name-only --diff-filter=ACMR"),
  );
  const staged = parseLines(
    run("git -c core.safecrlf=false diff --cached --name-only --diff-filter=ACMR"),
  );
