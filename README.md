...
- **Canonical URLs** - Self-referencing canonicals on content and list pages (`https://vibedeveloper.id/...`). The homepage and `/project/list` consolidate `?filter`/`?sort` variants onto their clean URLs. The root layout does not emit a homepage canonical, so child pages do not get two canonical tags. Site origin, host aliases, and WhatsApp links live in `lib/seo/site-url.ts` (`getSiteUrl` / `CANONICAL_SITE_ORIGIN`). `getSiteUrl` reads `process?.env` with optional chaining so an edge runtime that exposes `process` without `env` can still fall through to Worker bindings, Vite, or the production default. Alias hosts redirect to the apex (301 for GET/HEAD, 308 for other methods so POST server functions keep their method and body). DNS trailing-dot hosts (`vibedevid.com.`) are normalized before alias comparison.
