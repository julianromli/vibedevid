...
const AI_COMMANDS: AICommand[] = [
  ...
  },
  {
    name: "Make shorter",
    prompt:
      "Make the following text shorter and more concise while keeping the key information and meaning:",
  },
  {
    name: "Make longer",
    prompt:
    ...
    <div className="flex flex-col gap-2 border-muted border-t p-2">
      ...
      {!isLoading && response && (
        <div className="flex items-center gap-1">
          <Button size="sm" variant="ghost" onClick={onAccept} className="h-7 gap-1 px-2 text-xs">
            <Check className="h-3 w-3" />
            Accept
          </Button>
          <Button size="sm" variant="ghost" onClick={onReject} className="h-7 gap-1 px-2 text-xs">
            <X className="h-3 w-3" />
            Reject
          </Button>
          <Button size="sm" variant="ghost" onClick={onRetry} className="h-7 gap-1 px-2 text-xs">
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <Button
                ...
                variant="ghost"
                className={cn(
                  "h-8 gap-1 rounded-none border-none px-2 text-purple-500",
                  isOpen && "bg-accent",
                )}
              >
