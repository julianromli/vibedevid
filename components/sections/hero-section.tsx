...
export function HeroSection({ joinHref, handleViewShowcase }: HeroSectionProps) {
  ...
  const subtitleDelay =
    Math.min(Math.max(totalWords - 1, 0) * HERO_WORD_STAGGER_S, HERO_MAX_STAGGER_S) +
    HERO_SUBTITLE_DELAY_S;
        ...
        <div className="space-y-10 sm:space-y-12">
                ...
                <Fragment key={item.key}>
                  <MotionSpan
                    ...
                    transition={wordTransition(
                      getWordAnimationDelay(item.index + titleLine1.length),
                    )}
                  >
                    {item.word}
                  </MotionSpan>
          ...
          >
            <div className="my-0 flex items-center justify-center">
              <Suspense
                fallback={<div className="bg-muted/20 h-12 w-full animate-pulse rounded-lg" />}
              >
                <LogoMarquee />
              </Suspense>
            </div>
