...
export function getSafeRedirectPath(value: FormDataEntryValue | null | string | undefined): string {
  ...
  if (!trimmed.startsWith('/') || trimmed.startsWith('//') || trimmed.startsWith('/user/auth')) {
    return '/'
  }
...
export async function signUpWithCredentials(formData: FormData, origin: string): Promise<AuthCredentialsResult> {
...
export async function resetPasswordWithEmail(formData: FormData, origin: string): Promise<AuthCredentialsResult> {
  const email = formData.get('email')
