...
import { deleteBlogPostFn } from "@/lib/actions/blog.functions";
...
interface PostDashboardClientProps {
  initialPosts: Post[];
}

export function PostDashboardClient({ initialPosts }: PostDashboardClientProps) {
  ...
  const posts = useMemo(() => {
    ...
    return initialPosts.filter((post) => post.status === activeTab);
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteBlogPostFn({ data: { id: deleteId } });
      if (result.success) {
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete your blog post and remove
              all associated data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
