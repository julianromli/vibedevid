...
- **Option B (Scalable):** Update filters to use URL search params and trigger re-fetch (via router.refresh or passing searchParams to page).
  _Decision:_ Start with **Option A** (Client-side filtering of initial data) for smoother UX since the initial dataset will be small, but architect props to allow easy switch to Server-side filtering later.
